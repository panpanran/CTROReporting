...
namespace CTROLibrary.CTRO
{
    ...
    {
        public abstract string[] CreateBook(NpgsqlConnection conn, string startDate, string endDate, CTROLibrary.Model.Chart chart, out string[] Xaxis, out List<Dictionary<string, string>> Yaxis);
...
x.Field<int>("submissionnumber") == Convert.ToInt32(row["submissionnumber"]) && x.Field<string>("loginname") == row["loginname"].ToString());
        ...
        public override string[] CreateBook(NpgsqlConnection conn, string startDate, string endDate, CTROLibrary.Model.Chart chart, out string[] Xaxis, out List<Dictionary<string, string>> Yaxis)
        {
            //All

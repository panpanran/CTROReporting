...
(function($) {
    ...
    $.jqplot.MeterGaugeRenderer.prototype.init = function(options) {
        ...
        // incompatible col-md- and ticks are supplied, a suitable
        ...
        // col-md-, the sweep (in degrees) from min to max.  This gauge is 
        // a semi-circle.
        this.col-md- = 180;
        // get rid of this nonsense
        // this.innerSpan = this.col-md-;
        if (this.type == 'circular') {
            ...
            this.semiCircular = (this.col-md- <= 180) ? true : false;
        }
        this._tickPoints = [];
        // reference to label element.
        this._labelElem = null;
        
        // start the gauge at the beginning of the col-md-
        this.startAngle = (90 + (360 - this.col-md-)/2) * Math.PI/180;
        this.endAngle = (90 - (360 - this.col-md-)/2) * Math.PI/180;
    ...
    $.jqplot.MeterGaugeRenderer.prototype.draw = function (ctx, gd, options) {
        ...
        if (this.ticks.length == 0) {
            ...
            var max = this.max,
                ...
                setmax = this.setmax,
                setmin = this.setmin,
                ti = (max - min) * this.tickSpacing / this.col-md-;
        ...
        if (this.semiCircular) {
            ...
            if (this.showTicks) {
                ...
                var orad = this.tickOuterRadius,
                    ...
                    ts = this.col-md- * Math.PI / 180 / (this.ticks.length-1),
                    mts = ts/(nmt + 1);
            ...
            var start, end, col-md- = this.col-md-*Math.PI/180;
            for (i=0; i<this.intervals.length; i++) {
                start = (i == 0) ? sa : sa + (this.intervals[i-1][0] - this.min)*col-md-/range;
                if (start < 0) {
                    start = 0;
                }
                end = sa + (this.intervals[i][0] - this.min)*col-md-/range;
                if (end < 0) {
                    end = 0;
                }
                ctx.beginPath();
            ...
            if (this.pegNeedle) {
                if (this.data[0][1] > this.max + dataspan*3/this.col-md-) {
                    datapoint = this.max + dataspan*3/this.col-md-;
                }
                if (this.data[0][1] < this.min - dataspan*3/this.col-md-) {
                    datapoint = this.min - dataspan*3/this.col-md-;
                }
            }
            var dataang = (datapoint - this.min)/dataspan * this.col-md- * Math.PI/180 + this.startAngle;

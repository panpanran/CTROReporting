        ...
        public int CreateReport(string startDate, string endDate, string toemail, string reportname, out string savepath)
        {
            ...
            using (var conn = new NpgsqlConnection(CTRPConst.connString))
            {
                ...
                try
                {
                    ...
                    object[] parametersArray = new object[] { conn, startDate, endDate, outputParams, conclusionDS };
                    bookDS = methodInfo.Invoke(classInstance, parametersArray);
                    outputParams = (Dictionary<string, string>)parametersArray[3];
                    savepath = outputParams["savepath"];
                    templatepath = outputParams["templatepath"];
                    conclusionDS = (DataSet)parametersArray[4];
                    CTRPFunctions.WriteExcelByDataSet((DataSet)bookDS, savepath, templatepath, Convert.ToInt32(outputParams["startcell"].Split(',')[0]), Convert.ToInt32(outputParams["startcell"].Split(',')[1]));
                    CTRPFunctions.WriteExcelByDataSet(conclusionDS, savepath, null, Convert.ToInt32(outputParams["startcellconclusion"].Split(',')[0]), Convert.ToInt32(outputParams["startcellconclusion"].Split(',')[1]));
        ...
        public void CreateAnalysisChart(string startDate, string endDate, string toemail, string reportname, out string[] XLabel, out string[] YLabel, out string[] Xaxis, out string[] ChartName, out string[] ChartType, out List<Dictionary<string, string>> Yaxis, out string[] Loginname)
        {
            ...
            using (var conn = new NpgsqlConnection(CTRPConst.connString))
            {
                ...
                try
                {
                    ...
                    Dictionary<string, string> outputParams = new Dictionary<string, string>();
                    object[] parametersArray = new object[] { conn, startDate, endDate, outputParams, rankDS };
